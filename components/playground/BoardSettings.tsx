import { characters, useAICharacter } from "@/hooks/useAICharacter";

export const BoardSettings = () => {


  const teacher = useAICharacter((state) => state.character);
  const setTeacher = useAICharacter((state) => state.setCharacter);

  const classroom = useAICharacter((state) => state.classroom);
  const setClassroom = useAICharacter((state) => state.setClassroom);

  return (
    <>
      <div className="absolute right-0 bottom-full flex flex-row gap-10 mb-20">
        {characters.map((sensei, idx) => (
          <div
            key={idx}
